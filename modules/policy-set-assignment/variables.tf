...
locals {
  ...
  parameters = var.assignment_effect != null ? var.assignment_effect != "" ? jsonencode(merge(local.parameter_values, { effect = { value = var.assignment_effect } })) : jsonencode(local.parameter_values) : jsonencode(local.parameter_values)
  ...
  policy_definition_ids = length(local.role_definition_ids) > 0 ? toset([
    for d in var.initiative.policy_definition_reference :
    d.policy_definition_id
  ]) : []
